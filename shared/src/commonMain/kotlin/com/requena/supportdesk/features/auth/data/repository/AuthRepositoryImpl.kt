...
class AuthRepositoryImpl(
    ...
    override suspend fun claimClientAccess(code: String, name: String, email: String, password: String): AppResult<User> {
        return runCatching {
            val session = dataSource.claimClientAccess(code, name, email, password)
            sessionManager.persistSession(session)
        }.fold(
            onSuccess = { AppResult.Success(it) },
    ...
    private fun Throwable.toClaimMessage(): String {
        ...
        return when {
            ...
            rawMessage.contains("timeout", ignoreCase = true) ||
                rawMessage.contains("ConnectException", ignoreCase = true) ->
                "No se puede conectar con OryKai Server para activar el acceso. Backend configurado: ${supportDeskBaseUrl()}."
            rawMessage.isBlank() -> "No se pudo activar el acceso cliente."
            else -> rawMessage
        }
    }
}
